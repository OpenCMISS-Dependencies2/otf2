...
enum
{
    ...
    /* not used in any events, thus no create/destroy events */
...
#define WRITE_METRICS( new, t, rank, thread ) \
    ...
    if ( otf2_MPI )
    {
        ...
        snprintf( archive_path, sizeof( archive_path ),
                  "otf2_trace_gen_mpi_%" PRIu64 "_%" PRIu64 "%s",
    ...
    else if ( otf2_OPENMP )
    {
        ...
        snprintf( archive_path, sizeof( archive_path ),
                  "otf2_trace_gen_omp_%" PRIu64 "_%" PRIu64 "%s",
    ...
    else if ( otf2_HYBRID )
    {
        ...
        snprintf( archive_path, sizeof( archive_path ),
                  "otf2_trace_gen_mpi_omp_%" PRIu64 "x%" PRIu64 "_%" PRIu64 "%s",
    ...
    uint64_t initial_threads[ num_ranks ];
    ...
    for ( uint64_t rank = 0; rank < num_ranks; ++rank )
    {
        evt_writer = OTF2_Archive_GetEvtWriter( archive, initial_threads[ rank ] );
    ...
    for ( uint64_t rank = 0; rank < num_ranks; ++rank )
    {
        ...
        /* Enter MPI_Init. */
        status = OTF2_EvtWriter_Enter( evt_writer, attributes, t, REGION_MPI_INIT );
        check_status( status, "Write Enter." );

        status = OTF2_EvtWriter_MpiCollectiveBegin( evt_writer, NULL, t );
    ...
    for ( uint64_t rank = 0; rank < num_ranks; ++rank )
    {
        ...
        /* MPI_COMM_WORLD */
        status = OTF2_EvtWriter_CommCreate( evt_writer, NULL, t,
                                            rank /* each rank has its local communicator */ );
        check_status( status, "Write CommCreate." );

        /* MPI_COMM_SELF */
        status = OTF2_EvtWriter_CommCreate( evt_writer, NULL, t,
                                            rank + num_ranks /* each rank has its local communicator */ );
        check_status( status, "Write CommCreate." );
        ...
        status = OTF2_EvtWriter_MpiCollectiveEnd(
            ...
            rank,                      /* each rank has its local communicator */
            OTF2_COLLECTIVE_ROOT_NONE, /* MPI_Init has no root */
            0,
            0 );
        check_status( status, "Write MpiCollectiveEnd." );

        status = OTF2_EvtWriter_Leave( evt_writer, attributes, t, REGION_MPI_INIT );
    ...
    for ( uint64_t event = 0; event < otf2_EVENTS; ++event )
    {
        ...
        for ( uint64_t rank = 0; !otf2_OPENMP && rank < num_ranks; ++rank )
        {
            ...
            status = OTF2_EvtWriter_MpiCollectiveEnd(
                ...
                rank,                      /* each rank has its local communicator */
                OTF2_COLLECTIVE_ROOT_NONE, /* MPI_Barrier has no root */
                0,
                0 );
            check_status( status, "Write MpiCollectiveEnd." );
        ...
        if ( !otf2_MPI )
        {
            ...
            {
                evt_writer = OTF2_Archive_GetEvtWriter( archive, initial_threads[ rank ] );
                /* At this point the initial thread creates a team of several parallel threads */
            ...
            for ( uint64_t rank = 0; rank < num_ranks; ++rank )
            {
                 ...
                 * Now they synchronize and terminates. Only the initial thread continues execution. */
                t_leave = get_time();
    ...
    for ( uint64_t rank = 0; rank < num_ranks; ++rank )
    {
        ...
        /* Enter MPI_Finalize. */
        status = OTF2_EvtWriter_Enter( evt_writer, attributes, t, REGION_MPI_FINIALIZE );
        check_status( status, "Write Enter." );
    ...
    for ( uint64_t rank = 0; rank < num_ranks; ++rank )
    {
        ...
        /* MPI_COMM_WORLD */
        status = OTF2_EvtWriter_CommDestroy( evt_writer, NULL, t,
                                             rank /* each rank has its local communicator */ );
        check_status( status, "Write CommDestroy." );
        ...
        status = OTF2_EvtWriter_MpiCollectiveEnd(
            ...
            rank,                      /* each rank has its local communicator */
            OTF2_COLLECTIVE_ROOT_NONE, /* MPI_Finalize has no root */
            0,
            0 );
        check_status( status, "Write MpiCollectiveEnd." );

        status = OTF2_EvtWriter_Leave( evt_writer, attributes, t, REGION_MPI_FINIALIZE );
        check_status( status, "Write Leave." );
    }
    ...
    /* Write region definition for "MPI_Init". */
    status = OTF2_GlobalDefWriter_WriteString( glob_def_writer,
                                               string,
                                               "MPI_Init" );
    check_status( status, "Write string definition" );
    ...
    /* Write region definition for "MPI_Finalize". */
    status = OTF2_GlobalDefWriter_WriteString( glob_def_writer,
                                               string,
                                               "MPI_Finalize" );
    check_status( status, "Write string definition" );
